...
#include "utils/Adafruit_TSL2561_U.h"
...
//Configures the UART to run at 19200 baud rate
{
    SysCtlPeripheralEnable(SYSCTL_PERIPH_UART0);    //enables UART module 1
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOA);    //enables GPIO port b

    GPIOPinConfigure(GPIO_PA1_U0TX);    //configures PB1 as TX pin
    GPIOPinConfigure(GPIO_PA0_U0RX);    //configures PB0 as RX pin
...
//	x = I2C0_Read (TSL2591_ADDR, (TSL2591_COMMAND_BIT | TSL2591_REGISTER_DEVICE_ID));
	x = I2C0_Read (TSL2561_ADDR_LOW , (TSL2561_COMMAND_BIT | TSL2561_REGISTER_ID));
	if (x == 0x50)
	{
	...
	{
	    UARTprintf("device no found %i\n", x);
		while (1){};		//loop here if the dev ID is not correct
	}

	I2C0_Write (TSL2561_ADDR_LOW, 2, (TSL2561_COMMAND_BIT  | TSL2561_REGISTER_CONTROL), 0x10);	//configures the TSL2591 to have medium gain and integration time of 100ms
//	I2C0_Write (TSL2561_ADDR_LOW, 2, (TSL2561_COMMAND_BIT  | TSL2591_ENABLE), (TSL2591_ENABLE_POWERON | TSL2591_ENABLE_AEN | TSL2591_ENABLE_AIEN | TSL2591_ENABLE_NPIEN));	//enables proper interrupts and power to work with TSL2591
}
...
//This function will read the channels of the TSL and returns the calculated value to the caller
{
	...
	x = I2C0_Read (TSL2561_ADDR_LOW, (TSL2561_COMMAND_BIT | TSL2561_REGISTER_CHAN0_HIGH));
	x <<= 16;
	x |= I2C0_Read (TSL2561_ADDR_LOW, (TSL2561_COMMAND_BIT | TSL2561_REGISTER_CHAN0_LOW));
